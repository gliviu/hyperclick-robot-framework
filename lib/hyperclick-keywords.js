...
function getKeywordFromImports(keywordInfo, imports, autocompleteRobotProvider){
  const keywordName = keywordInfo.keywordName
  const prefix = keywordInfo.prefix
  ...
  getKeywordSuggestions(textEditor, point, autocompleteRobotProvider){
    ...
    if(activeResource) {
      for(keywordInfo of keywordInfoItems) {
        highlightedKeywords = highlightedKeywords.concat(
            getKeywordFromImports(keywordInfo, activeResource.imports, autocompleteRobotProvider))
      }
    }
