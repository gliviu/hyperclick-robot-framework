...
function getKeywordFromImports(keywordName, imports){
  ...
  for(let importedResource of imports.resources){
    if(importedResource.resourceKey){
      ...
      for(const resourceKey of resourceKeys){
        ...
        if(resource.name === importedResource.name){
          matchingImportedResources.add(resource)
        }
      }
    }
...
export default {
  ...
  getSuggestion(textEditor, point){
    ...
    const activeResource = getCurrentResource(textEditor.getPath())
    if(activeResource){
      highlightedKeywords = getKeywordFromImports(keywordInfo.keywordName, activeResource.imports)
    }
